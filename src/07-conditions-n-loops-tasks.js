...
function doRectanglesOverlap(rect1, rect2) {
  ...
  if ((rect1.top >= rect2.top && rect1.top <= rect2.top + rect2.height)
    ...
    || (rect2.top + rect2.height >= rect1.top
        && rect2.top + rect2.height <= rect1.top + rect1.height)) yOver = true;
  ...
  || (rect2.left + rect2.width >= rect1.left
    && rect2.left + rect2.width <= rect1.left + rect1.width)) xOver = true;
  return xOver && yOver;
}
...
function findFirstSingleChar(str) {
  ...
  return str.split('').find((x) => letters.filter((y) => x === y).length === 1);
}
 ...
 */
function getDigitalRoot(numm) {
  let num = numm.toString();
  const sum = (n) => n.split('').reduce((acc, x) => acc + parseInt(x, 10), 0);
  while (sum(num) > 9) num = sum(num).toString();
  return sum(num);
}
